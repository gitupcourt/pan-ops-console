...
class PanoramaClient:
    ...
    def __init__(self, hostname: str, api_key: str, *, verify_tls: bool = True):
        """The merged app stores Fernet-encrypted API keys inline on the
        Panorama row (capacity's pattern). Callers decrypt with
        `core.credentials.decrypt_key` and pass the plaintext key here.
        Username/password is no longer accepted at this layer — the
        userpass → keygen exchange happens at Panorama-create time
        (see `core.command_proxy.pan_client.keygen`)."""
