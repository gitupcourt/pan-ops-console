...
Both paths produce a working FirewallProxy that the rest of the code uses
identically.
"""
...
class PanDeviceClient:
    ...
    def direct(cls, host: str, cred: ResolvedCredential, *, verify_tls: bool = True) -> "PanDeviceClient":
        """Talk straight to the device's mgmt IP/hostname using its own credential."""
        if cred.api_key:
            proxy = FirewallProxy(hostname=host, api_key=cred.api_key)
        else:
            proxy = FirewallProxy(
                hostname=host,
                api_username=cred.username,
                api_password=cred.password,
            )
    ...
    def via_panorama(
        ...
        *,
        ...
        Build a Panorama with creds, attach a Firewall(serial=...) child so its
        op() calls flow through Panorama, then wrap that Firewall in a FirewallProxy
        so pan-os-upgrade-assurance can use it.
        """
