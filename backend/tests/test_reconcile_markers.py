...
def stub_probe_and_client(monkeypatch):
    ...
    Two subtle things:
      - `from app.services import precheck as precheck_svc` (local import
        inside reconcile_markers_...) resolves to the PACKAGE attribute
        already loaded on app.services — patching sys.modules wouldn't
        intercept it. We patch the real module attribute instead.
    ...
    from app.services import precheck as real_precheck  # noqa: PLC0415
